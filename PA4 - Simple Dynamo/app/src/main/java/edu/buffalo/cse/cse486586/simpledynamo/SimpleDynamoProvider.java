		...
		if (key.equalsIgnoreCase("*")) {
			...
			for (String port: targetPorts) {
				try {
					...
					if(queryResult.equals("DEFAULT")){
						Log.e(TAG, "Response arrived.... queryResult : " + queryResult + "Continuing querying to next port...");
						continue;
					}else{
						Log.e(TAG, "Response arrived.... queryResult : " + queryResult);
						break;
					}
				} catch (Exception e) {
			...
			String[] row = new String[]{key, queryResult};
			cursor.addRow(row);
			returnCursor = cursor;
		...
		for (String port: REMOTE_PORTS) {
			...
			try {
				...
				for (Message msg: messageList) {
					...
					messageMap.put(msg.getKey(), msg.getValue());
				}
			} catch (Exception e) {
